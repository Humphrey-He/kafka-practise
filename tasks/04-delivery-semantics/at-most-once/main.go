...
func main() {
	var (
		...
		groupID    = flag.String("group", "kp_at_most_once_group", "consumer group id")
	)
	flag.Parse()

	logger := logging.New()
	if err := run(*configPath, []string{*topic}, *groupID, logger); err != nil {
		logger.Error("at-most-once consumer failed", slog.String("error", err.Error()))
		os.Exit(1)
	}
}
...
func run(configPath string, topics []string, groupID string, logger *slog.Logger) error {
	...
	handler := &atMostOnceHandler{logger: logger}
	for ctx.Err() == nil {
...
func (h *atMostOnceHandler) Setup(sarama.ConsumerGroupSession) error { return nil }

func (h *atMostOnceHandler) Cleanup(sarama.ConsumerGroupSession) error { return nil }
...
func (h *atMostOnceHandler) ConsumeClaim(session sarama.ConsumerGroupSession, claim sarama.ConsumerGroupClaim) error {
	for message := range claim.Messages() {
		...
		h.logger.Warn("offset committed before business processing",
