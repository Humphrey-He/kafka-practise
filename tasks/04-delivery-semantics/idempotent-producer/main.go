...
func main() {
	var (
		configPath = flag.String("config", "configs/idempotent.yaml", "config file path")
		topic      = flag.String("topic", "kp_topic_demo", "topic name")
		key        = flag.String("key", "idempotent-key", "message key")
		value      = flag.String("value", "idempotent message", "message value")
	...
	if err := run(*configPath, *topic, *key, *value, *count, logger); err != nil {
		logger.Error("idempotent produce failed", slog.String("error", err.Error()))
		os.Exit(1)
	}
}
...
func run(configPath, topic, key, value string, count int, logger *slog.Logger) error {
	...
	appCfg.Kafka.Producer.Idempotent = true
	...
	for i := 0; i < count; i++ {
		...
		logger.Info("idempotent message sent",
